...
from keras.layers import Input, LSTM, Dense, Embedding, TimeDistributed, Dropout, BatchNormalization, Bidirectional, Concatenate
...
class seq2seq_all():
    ...
    def __init__(self, weight_name, data_name):
        ...
        data_path = file_path + '/' + self.data_name
...
#        print('Max sequence length for outputs:', self.max_decoder_seq_length)
        ...
        self.input_token_index['<PAD>'] = 0
        self.target_token_index['<PAD>'] = 0
        
      
        for i, char in enumerate(input_characters):
            self.input_token_index[char] = i + 1
        
        for  i, char in enumerate(target_characters):
            self.target_token_index[char] = i + 1
...
#訓練
    def build_basic_model(self):
        ...
        encoder_lstm = Bidirectional(LSTM(self.latent_dim, return_state=True), name='BI_encoder_lstm')
        ...
        decoder_lstm = LSTM(self.latent_dim*2, return_state=True, return_sequences=True, name='BI_decoder_lstm' )
        rnn_outputs, *decoder_states = decoder_lstm(decoder_embedding, initial_state=encoder_states)
        decoder_dense = Dense(self.num_decoder_tokens, activation='softmax', name='decoder_dense')
        decoder_outputs = decoder_dense(BatchNormalization(name='BN1')(rnn_outputs))
    
        basic_model = Model([encoder_inputs, decoder_inputs], [decoder_outputs])
        basic_model.compile(optimizer='Adam', loss='sparse_categorical_crossentropy', metrics=['sparse_categorical_accuracy'])
        basic_model.summary()
        
        #繪圖
        plot_model(basic_model, to_file='model.png', show_shapes=True)
        ...
        model = basic_model.fit_generator(self.generate_batch_data_random(train_data, teach_data, train_data_y, self.BATCH_SIZE),
...
#                        validation_split=0.2, callbacks=callback_list)
        ...
        basic_model.save_weights(self.weight_name)
    ...
    def creat_model(self):
        ...
        decoder_inputs = Input(shape=(None,), name='decoder_inputs')
        decoder_embedding = Embedding(self.num_decoder_tokens, 200, name='decoder_embedding')(decoder_inputs)
        decoder_lstm = LSTM(self.latent_dim*2, return_state=True, return_sequences=True, name='decoder_lstm' )
        rnn_outputs, *decoder_states = decoder_lstm(decoder_embedding, initial_state=encoder_states)
        decoder_dense = Dense(self.num_decoder_tokens, activation='softmax', name='decoder_dense')
        decoder_outputs = decoder_dense(BatchNormalization(name='BN1')(rnn_outputs))
    ...
    def build_basic_inference_model(self):
        ...
        model.load_weights(file_path + "/" + self.weight_name)
        ...
        # get encoder states
        _, forward_h, forward_c, backward_h, backward_c = model.get_layer('BI_encoder_lstm')(encoder_embedding)
        state_h = Concatenate()([forward_h, backward_h])
        state_c = Concatenate()([forward_c, backward_c])
        encoder_states = [state_h, state_c]
        ...
        decoder_state_h = Input(shape=(self.latent_dim*2,))
        decoder_state_c = Input(shape=(self.latent_dim*2,))
        ...
        BN = model.get_layer('BN1')(rnn_outputs)
        
        decoder_outputs = model.get_layer('decoder_dense')(BN)
    ...
    def decode_sequence(self, input_seq, encoder_model, decoder_model):
        ...
        with graph.as_default():
            ...
            while not stop_condition:
                ...
                # Sample a token
                sampled_token_index = self.sample(output_tokens[0, -1, :])
...
x = seq2seq_all('P1_big_BN.h5', 'QA_all.txt')
x.build_basic_model()
###
while True:
